...
eigenvalue with an even eigenfunction~\cite{Suzuki2026WeilScrew}. Chuk reports
an unconditional certificate through $a=0.8$, equivalently
$\lambda^2=e^{2a}\approx4.95$, including a simple even continuum ground
state~\cite{Chuk2026CompactWeil}. These analytic ranges remain below the
finite full-space configurations tested here, which begin at
$\lambda^2=13$; they do not exclude a later even--odd crossing. Chuk also
...
\noindent The tabulated numerical results are the historical version 2.4
measurements, obtained with Xcelerator Toolkit version 0.14.2. Version 2.5
upgrades the reproduction harness to Toolkit 0.15.0, with an exact commit
recorded in Cargo.lock. Individual claim scripts now retain primary results,
source-bound Ultra capture receipts, and explicit numerical Pass/Fail summaries.
The harness upgrade does not by itself constitute a rerun of every table;
new run journals distinguish fresh measurements from the historical results.
...
\bibitem{Chuk2026CompactWeil}
M.~Chuk,
\emph{Weil Positivity in Compact Windows: Certified Two-Sided Bounds
and a Landau--Widom Decay Law},
arXiv:2608.24827v1 (2026).
